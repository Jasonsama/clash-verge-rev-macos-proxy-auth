...
fn apply_proxy_state(sys: &Sysproxy, auto: &Autoproxy, apply_steps: [ProxyApplyStep; 2]) -> Result<()> {
    let apply = || -> sysproxy::Result<()> {
    ...
    match apply() {
        Ok(()) => Ok(()),
        #[cfg(target_os = "macos")]
        Err(sysproxy::Error::RequiresAdminPrivileges) => {
            logging!(
                info,
                Type::Core,
                "Requesting administrator authorization to modify system proxy"
            );
            super::macos_sysproxy::apply_with_authorization(sys, auto, apply_steps[0] == ProxyApplyStep::Autoproxy)
        }
...
impl Sysopt {
    ...
    pub async fn update_sysproxy(&self) -> Result<()> {
        ...
        #[cfg(target_os = "macos")]
        if super::macos_sysproxy::apply_via_service(&sys, &auto, apply_steps[0] == ProxyApplyStep::Autoproxy).await? {
            return Ok(());
        }

        tokio::task::spawn_blocking(move || apply_proxy_state(&sys, &auto, apply_steps)).await??;
    ...
    pub async fn reset_sysproxy(&self) -> Result<()> {
        ...
        #[cfg(target_os = "macos")]
        if super::macos_sysproxy::apply_via_service(&sys, &auto, false).await? {
            return Ok(());
        }

        tokio::task::spawn_blocking(move || {
            apply_proxy_state(&sys, &auto, [ProxyApplyStep::Sysproxy, ProxyApplyStep::Autoproxy])
        })
        .await??;
