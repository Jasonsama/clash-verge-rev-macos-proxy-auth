...
use crate::{
    ClashConfig, IPC_AUTH_EXPECT, IPC_PATH, IpcCommand, ServiceStatusSnapshot, SystemProxyConfig, WriterConfig,
...
pub async fn apply_system_proxy(body: &SystemProxyConfig) -> Result<Response<()>> {
    let client = connect().await?;
    let payload = body.to_json_value()?;
    let response = client
        .put(IpcCommand::ApplySystemProxy.as_ref())
        .json_body(&payload)
        .header(IPC_AUTH_HEADER_KEY, IPC_AUTH_EXPECT)
        .send()
        .await?
