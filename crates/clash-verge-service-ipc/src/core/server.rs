...
use crate::{ClashConfig, IpcCommand, SystemProxyConfig, VERSION, WriterConfig};
...
#[cfg(target_os = "macos")]
fn apply_system_proxy(config: &SystemProxyConfig) -> AnyResult<()> {
    use std::process::Command;

    if config.service.is_empty()
        || config.service.len() > 256
        || config.service.chars().any(char::is_control)
        || config.host.is_empty()
        || config.host.len() > 255
        || config.host.chars().any(char::is_control)
        || config.auto_url.len() > 2048
        || config.auto_url.chars().any(char::is_control)
        || config.bypass.len() > 256
        || config
            .bypass
            .iter()
            .any(|domain| domain.len() > 255 || domain.chars().any(char::is_control))
    {
        return Err(anyhow!("invalid system proxy configuration"));
    }

    let run = |args: &[String]| -> AnyResult<()> {
    ...
    let state = if config.system_enabled { "on" } else { "off" };
    ...
    for (set, toggle) in [
        ...
        system_commands.push(vec![
            set.into(),
            config.service.clone(),
            config.host.clone(),
            config.port.to_string(),
        ]);
        system_commands.push(vec![toggle.into(), config.service.clone(), state.into()]);
    }
    let mut bypass = vec!["-setproxybypassdomains".into(), config.service.clone()];
    bypass.extend(config.bypass.iter().cloned());
    system_commands.push(bypass);

    let auto_commands = vec![
        vec![
            "-setautoproxyurl".into(),
            config.service.clone(),
            config.auto_url.clone(),
        ],
        vec![
            "-setautoproxystate".into(),
            config.service.clone(),
            if config.auto_enabled { "on" } else { "off" }.into(),
        ],
    ];
    let commands: Vec<Vec<String>> = if config.auto_first {
        auto_commands.into_iter().chain(system_commands).collect()
    } else {
        system_commands.into_iter().chain(auto_commands).collect()
    };
...
pub async fn run_ipc_server() -> Result<JoinHandle<Result<()>>> {
    ...
    if let Some(mut server) = IpcState::global().take_server().await {
        ...
        #[cfg(unix)]
        {
            ...
            if socket_ready {
                fs::set_permissions(paths.ipc_path(), Permissions::from_mode(0o770)).await?;
            } else {
                warn!(
...
fn create_ipc_router() -> Result<Router> {
    let router = Router::new()
        ...
        .put(IpcCommand::ApplySystemProxy.as_ref(), |ctx| async move {
            ipc_request_context_to_auth_context(&ctx)?;
            #[cfg(target_os = "macos")]
            {
                let config = match ctx.json::<SystemProxyConfig>() {
                    Ok(config) => config,
                    Err(error) => {
                        return Ok(HttpResponse::builder()
                            .status(StatusCode::BAD_REQUEST)
                            .text(format!("Invalid JSON: {error}"))
                            .build());
                    }
                };
                match tokio::task::spawn_blocking(move || apply_system_proxy(&config)).await {
                    Ok(Ok(())) => ok_empty("System proxy updated"),
                    Ok(Err(error)) => service_unavailable(error.to_string()),
                    Err(error) => service_unavailable(error.to_string()),
                }
            }
            #[cfg(not(target_os = "macos"))]
            service_unavailable("System proxy service is only available on macOS".to_owned())
        });
    Ok(router)
}
